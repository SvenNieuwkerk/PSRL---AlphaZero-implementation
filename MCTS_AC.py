...
class MCTSPlanner_AC:
    ...
    def expand_child(self, node: "MCTSNode", depth: int) -> Optional[List[Tuple["Child", float, bool]]]:
        ...
        for _ in range(attempts):
            ...
            # Transition via via AC step_fn
            next_state, next_coin_collected, reward, done, info = self.step_fn(node, a_star)
            reward = float(reward)
            done = bool(done)

            # Detect adaptation
            adapted = info.get("adapted_action", None)
            was_adapted = False
            adapted_action = None

            if adapted is not None:
                adapted_action = np.asarray(adapted, dtype=np.float32)
                a_star_f = a_star.astype(np.float32)
                was_adapted = (np.linalg.norm(adapted_action - a_star_f) > self.adapt_eps)
            ...
            if was_adapted:
                ...
                # ---- (2) Safe branch: adapted action => real transition ----
                safe_child = node.add_child(
                    action=adapted_action.astype(np.float32),
                    child_state=next_state,
                    prior_raw=prior_raw,
                    reward=reward,
                )
                safe_child.coin_collected = bool(next_coin_collected)
                safe_edge = node.children[-1]

                if done:
                    safe_child.is_terminal = True
                    safe_child.terminal_value = float(self.terminal_mapping(reward, info))

                # Normalize priors
                self.normalize_node_priors(node)

                return [(unsafe_edge, penalty, True), (safe_edge, reward, done)]
            
            # ---- Normal case: no adaptation => single child ----
            # Add child
